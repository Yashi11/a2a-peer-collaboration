...
This package proposes an opt-in extension for teams of equal agent peers. Each
...
per-answer requester receipts. Receipts record whether a particular answer was
used or rejected, producing an auditable collaboration trace without exposing
private reasoning.
...
It validates task ownership and artifact digests, but intentionally does **not**
grant tool permissions, execute artifacts, merge code, or implement an A2A
transport. It captures aggregate process telemetry—learning shares, requests,
claims, answers, used/rejected receipts, mean response latency, and trace-event
count—without preserving hidden reasoning.
